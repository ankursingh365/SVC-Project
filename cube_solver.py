...
class IDA_star(object):
    ...
    def run(self, state):
        """
        Input: state - string representing the current state of the struct
        Description: solve the rubix struct
        Output: list containing the moves taken to solve the struct
        """
        while True:
    ...
    def search(self, state, g_score):
        ...
        Output: boolean representing if the struct has been solved
        """
        struct = RubiksCube(state=state)
        if struct.solved():
            return True
        ...
        min_val = float('inf')
        best_action = None
        for a in [(r, n, d) for r in ['h', 'v', 's'] for d in [0, 1] for n in range(struct.n)]:
            struct = RubiksCube(state=state)
            if a[0] == 'h':
                struct.horizontal_twist(a[1], a[2])
            elif a[0] == 'v':
                struct.vertical_twist(a[1], a[2])
            elif a[0] == 's':
                struct.side_twist(a[1], a[2])
            if struct.solved():
                self.moves.append(a)
                return True
            cube_str = struct.stringify()
...
def build_heuristic_db(state, actions, max_moves = 20, heuristic = None):
    ...
    Description: create a heuristic map for determining the best path for solving a rubiks struct
